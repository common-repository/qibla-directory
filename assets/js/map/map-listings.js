...
(
    ...
    )
    {
        ...
        DL.Listings.Filter = {
            ...
            visibleMap: function ()
            {
                return dllistings.mapVisible || this.useScMap();
            },
            ...
            init: function ()
            {
                if (this.visibleMap()) {
                    ...
                    DlMap.map = new DlMap.Map(document.querySelector('#dlgoogle-map'), dlgooglemap);
        ...
        DL.Listings.FilterToggler = {
             ...
             * @since 1.0.0
             */
            createTogglerEL: function ()
            {
                if (this.filters) {
                    // The Fragment.
                    var frag           = document.createDocumentFragment(),
                        togglerWrapper = document.createElement('div'),
                        toolbar        = document.querySelector('#dllistings_toolbar'),
                        toggler        = document.querySelector('#dltoggler_filter');

                    // Set the toggler wrapper.
                    togglerWrapper.setAttribute('id', 'dltogglers');
                    togglerWrapper.classList.add('dltogglers');

                    // Append the elements.
                    frag.appendChild(togglerWrapper);
                    togglerWrapper.insertAdjacentHTML('beforeend', toggler.innerHTML);
            ...
            init: function ()
            {
                var filterTogglerEl = this.container.querySelector('#dltoggler_filters');
