...
<li id="comment-<?php comment_ID(); ?>"
    class="<?php echo \QiblaDirectory\Functions\sanitizeHtmlClass($data->containerClass) ?>">
    <article id="div-comment-<?php echo esc_attr($data->ID) ?>" class="dlreview comment-body">

        <figure class="dlreview__thumbnail comment-thumbnail">
            <?php echo F\ksesImage($data->avatarMarkup) ?>

            <?php if ($data->avatarMarkup) : ?>
                <figcaption class="dlreview__author comment-author">
                    <?php echo esc_html(sanitize_text_field($data->author)) ?>
                </figcaption>
            <?php endif; ?>
        </figure>

        <section class="dlreview__content comment-content">
            ...
            <footer class="dlreview__meta comment-metadata">
                <?php
                // Show Rating.
            ...
            <div class="dlreview__content description">
                <?php if ('spam' !== $data->status && 0 === intval($data->status)) : ?>
                    <p class="comment-awaiting-moderation">
                        <?php esc_html_e('Your Review is awaiting moderation.', 'qibla-directory'); ?>
                    </p>
                    <?php
                elseif (1 === intval($data->status)) :
                    ...
                    if ($data->replyLink) : ?>
                        <a class="dlbtn dlbtn--tiny comment-reply-link"
