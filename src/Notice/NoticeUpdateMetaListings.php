...
namespace QiblaDirectory\Notice;
...
class NoticeUpdateMetaListings
{
    ...
    public static function noticeFilter()
    {
        ...
        if ('1' !== \QiblaDirectory\Functions\filterInput($_GET, 'uoml', FILTER_SANITIZE_NUMBER_INT)) {
            return;
        }
        ...
        <section class="notice notice-error" style="padding: 1em; margin-left: 0;">
            <h1><?php esc_html_e('Qibla Update', 'qibla-directory') ?></h1>
            ...
            <article>
                ...
                <p>
                    <?php echo \QiblaDirectory\Functions\ksesPost(__(
                ...
                <p>
                    <?php echo \QiblaDirectory\Functions\ksesPost(sprintf(__(
                        'You have %s listings stored.', 'qibla-directory'
                    ), '<strong>' . $count . '</strong>')); ?>

                    <?php if (count($invalid)) {
                        echo \QiblaDirectory\Functions\ksesPost(sprintf(__(
                            'And %s need a manual update.', 'qibla-directory'
                        ), '<strong>' . count($invalid) . '</strong>'));
                    } ?>

                    <?php if (500 < $count) : ?>
                        <span><?php _e('This may take a while. Please, be patient.', 'qibla-directory') ?></span>
                    <?php endif; ?>
                </p>
                ...
                $trigger = new \QiblaDirectory\Task\TaskRefactorListingsMetaLocationTriggerTemplate();
