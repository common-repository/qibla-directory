 ...
 *
 * @since      1.0.0
 * @package    QiblaDirectory\Front\Settings
...
final class AmenitiesTemplate implements T\TemplateInterface
{
    ...
    private function getTermsIconLabel(array $terms)
    {
        ...
        foreach ($terms as $term) {
            ...
            $list[$term->slug] = array(
                ...
                'label'           => $term->name,
                'href'            => get_term_link($term, 'amenities'),
            );
        }
    ...
    public function getData()
    {
        ...
        // Section Title.
        $data->title = esc_html__('Amenities', 'qibla-directory');
        // Retrieve the terms.
        $data->terms = get_the_terms($this->post->ID, 'amenities');
    ...
    {
        if ($data->terms) {
            $engine = new T\Engine('single_listings_amenities', $data, 'views/terms/amenities.php');
            $engine->render();
        }
    }
