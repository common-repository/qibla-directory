 ...
 * Post Functions
 *
 * @package QiblaDirectory\Functions
...
use QiblaDirectory\IconsSet\Icon;
...
use QiblaDirectory\TemplateEngine\Engine;
 ...
 * refer to `QiblaDirectory\Thumbnail`.
 *
 * @since 1.0.0
 *
...
function getExcerptData()
{
    ...
    if ($theContent) {
        ...
        $data->moreLinkLabel = esc_html__('Continue Reading', 'qibla-directory');
    }
...
function getListingsTermsList($post = null)
{
    ...
    // Retrieve the terms.
    $terms = get_the_terms($post, 'listing_categories');
...
function listingsTermsListTmpl()
{
    ...
    try {
        $data = call_user_func_array('QiblaDirectory\\Functions\\getListingsTermsList', func_get_args());
...
function thePostThumbnailTmpl()
{
    ...
    try {
        // Retrieve the data.
        $data = call_user_func_array('QiblaDirectory\\Functions\\getPostThumbnailData', func_get_args());
...
function getPostPubDateData($post = null)
{
    ...
    $data->label           = esc_html__('Published On: ', 'qibla-directory');
...
{
    try {
        $data = call_user_func_array('QiblaDirectory\\Functions\\getPostPubDateData', func_get_args());
    } catch (\Exception $e) {
        $debugInstance = new Debug\Exception($e);
        'dev' === QB_ENV && $debugInstance->display();
...
{
    try {
        $data = call_user_func_array('QiblaDirectory\\Functions\\getPostTermsData', func_get_args());
    } catch (\Exception $e) {
        $debugInstance = new Debug\Exception($e);
        'dev' === QB_ENV && $debugInstance->display();
...
function loopFooter($post = null, $args = array())
{
    ...
    // Default condition.
    $isPostType = false;
    if (class_exists('QiblaDirectory\\ListingsContext\\Types')) {
        $types      = new Types();
    ...
    $engine = new Engine('post_loop_footer', $data, '/views/posts/loopFooter.php');
    $engine->render();
}
...
function linkPages()
{
    ...
    add_filter('wp_link_pages_link', function ($markup, $i) {
        ...
        if (false === strpos($markup, '<a')) {
            ...
            $markup = '<span class="screen-reader-text">' . esc_html__('Page', 'qibla-directory') . '</span>';
    ...
    $data->linkPagesArgs = array(
        ...
        'after'       => '</ul>',
        'link_before' => '<span class="screen-reader-text">' . esc_html__('Page', 'qibla-directory') . '</span>',
        'link_after'  => '',
    );
