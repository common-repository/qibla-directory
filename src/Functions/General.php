...
use QiblaDirectory\Front\Settings\Listings;
...
function svgLoaderTmpl()
{
    if (is_admin()) {
        ...
        if ('qibla-directory-options' !== $screen->parent_base) {
            return;
        }
    }
...
{
    $query   = \QiblaDirectory\Functions\getWpQuery();
...
function getScopeClass($block = '', $element = '', $modifier = '', $attr = 'class')
{
    ...
    if ($block) :
        ...
        if (is_author()) {
             ...
             */
            $upxscope .= apply_filters('qibla_directory_get_singular_scope_post_type', $postType, $upxscope);
        } elseif (is_404()) {
            $upxscope .= '404';
        }//end if
    endif;
     ...
     */
    $upxscope = apply_filters('qibla_directory_scope_attribute', $upxscope, $element, $block, $scope, $attr, $modifier);
...
/**
 * Check Qibla Listings
 *
 * @since 1.0.0
 *
 * @return bool True if check pass, false otherwise
 */
function checkQiblaListings()
{
    if (! function_exists('is_plugin_active')) {
        require_once untrailingslashit(ABSPATH) . '/wp-admin/includes/plugin.php';
    }

    return is_plugin_active('qibla-listings/index.php');
}
...
{
    if (checkQiblaFramework() || checkQiblaListings()) :
        add_action('admin_notices', function () {
            ?>
            <div class="error">
                <p>
                    <?php esc_html_e(
                        'Qibla Directory has been deactivated. The plugin can not be activated if active Qibla Framework and Qibla Listings.',
                        'qibla-directory'
                    ); ?>
                </p>
            </div>
        ...
        deactivate_plugins('qibla-directory/index.php');
    endif;
}
...
function setViewedCookie()
{
    ...
    if (in_array(get_post_type(), $types->types(), true)) :
        ...
        if (! isset($_COOKIE['qibla_directory_listings_recently_viewed'])) {
            // If there's no cookie set, set up a new array.
            $cookieArray = array($postID);
        } else {
            $cookieArray = unserialize($_COOKIE['qibla_directory_listings_recently_viewed']);
        ...
        // Set cookie.
        setcookie(
            'qibla_directory_listings_recently_viewed',
