...
use QiblaDirectory\Front\Settings;
...
function getFoundPosts()
{
    ...
    $data->label  = esc_html__('results', 'qibla-directory');
    ...
    $data->numSeparator = esc_html_x('of', 'found-posts-separator', 'qibla-directory');
...
function getTheArchiveDescription($queriedObject = null)
{
    ...
    if (is_tax() || is_tag() || is_category()) {
        ...
        // Translators: The %s is the i18n numbers of the posts found.
            esc_html__('We had found %s posts based on your search', 'qibla-directory'),
...
function getArchivePagination()
{
    ...
    $data->paginateArgs = array(
        ...
        'before_page_number' => '<span class="screen-reader-text">' . esc_html__('Page', 'qibla-directory') . '</span>',
        'prev_text'          => sprintf(
            '<i class="fa fa-chevron-left" aria-hidden="true"></i>%s',
            '<span class="screen-reader-text">' . esc_html__('Previous Page', 'qibla-directory') . '</span>'
        ),
        'next_text'          => sprintf(
            '%s<i class="fa fa-chevron-right" aria-hidden="true"></i>',
            '<span class="screen-reader-text">' . esc_html__('Next Page', 'qibla-directory') . '</span>'
        ),
    );
...
{
    $data = call_user_func_array('QiblaDirectory\\Functions\\getArchivePagination', func_get_args());
