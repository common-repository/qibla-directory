...
class Listings extends AbstractPostType
{
    ...
    public function __construct()
    {
        parent::__construct(
            'listings',
            esc_html__('Listing', 'qibla-directory'),
            esc_html__('Listings', 'qibla-directory'),
            array(
                ...
                'has_archive'     => true,
                'description'     => F\getPluginOption('listings', 'listings_archive_description', true),
    ...
    public function columns($columns)
    {
        ...
        $columns = array_merge($columns, array(
            'location' => esc_html_x('Location', 'admin-table-list', 'qibla-directory'),
            'category' => sprintf(
                '<span class="has-icon">%s</span>',
                esc_html_x('Category', 'admin-table-list', 'qibla-directory')
            ),
//            'amenities' => esc_html_x('Amenities', 'admin-table-list', 'qibla-directory'),
            'featured' => sprintf(
                '<span class="has-icon">%s</span>',
                esc_html_x('Featured', 'admin-table-list', 'qibla-directory')
            ),
//            'gallery'  => esc_html__('Gallery', 'qibla-directory'),
            'creator'  => sprintf(
                '<span class="has-icon" data-icon="">%s</span>',
                esc_html_x('Author', 'admin-table-list', 'qibla-directory')
            ),
            'comments' => sprintf(
                '<span class="has-icon">%s</span>',
                esc_html_x('Reviews', 'admin-table-list', 'qibla-directory')
            ),
        ));
...
//                    '<span>' . esc_html__('Yes', 'qibla-directory') . '</span><i class="la la-check la--active">' :
//                    '<span>' . esc_html__('No', 'qibla-directory') . '</span><i class="la la-check">';
//                break;
            case 'category':
                $terms = get_the_term_list($post->ID, 'listing_categories', '', ',');
...
//                break;
            case 'featured':
                ...
                echo 'on' === $meta ?
                    '<span>' . esc_html__('Yes', 'qibla-directory') .
                    '</span><i class="la la-star dlicon dlicon--active">' :
                    '<span>' . esc_html__('No', 'qibla-directory') .
