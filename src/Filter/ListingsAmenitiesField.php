...
class ListingsAmenitiesField implements FilterFieldInterface
{
    ...
    private function value()
    {
        ...
        if (! $value) {
            ...
            $value = $qObj instanceof \WP_Term && is_tax('amenities') ? $qObj->slug : '';
        }
    ...
    private function filterOptionsBasedOnTaxonomy()
    {
        ...
        if (! $currObj instanceof \WP_Term ||
            (isset($currObj->taxonomy) && 'listing_categories' !== $currObj->taxonomy)
        ) {
        ...
        $list = F\getTermsByTermBasedOnContext(get_terms('amenities'), 'listing_categories');
