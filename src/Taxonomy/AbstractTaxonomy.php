    ...
    protected function __construct($name, $postType, $singular, $plural, array $args = array())
    {
        ...
        $this->args     = wp_parse_args($args, array(
            ...
            'labels'  => array(
                ...
                'update_item'               => sprintf(esc_html__('Update %s', 'qibla-directory'), $singular),
                'add_new_item'              => sprintf(esc_html__('Add New %s', 'qibla-directory'), $singular),
                'new_item_name'             => sprintf(esc_html__('New %s', 'qibla-directory'), $singular),
                'parent_item'               => sprintf(esc_html__('Parent %s', 'qibla-directory'), $singular),
                'parent_item_column'        => sprintf(esc_html__('Parent %s:', 'qibla-directory'), $singular),
                'search_items'              => sprintf(esc_html__('Search %s', 'qibla-directory'), $plural),
                'popular_items'             => sprintf(esc_html__('Popular %s', 'qibla-directory'), $plural),
                'separate_items_with_comma' => sprintf(
                    esc_html__('Separate %s with comma', 'qibla-directory'),
                    $plural
                ),
                'add_or_remove_items'       => sprintf(esc_html__('Add or remove %s', 'qibla-directory'), $plural),
                'choose_from_most_used'     => sprintf(
                    esc_html__('Choose from most used %s', 'qibla-directory'),
                    $plural
                ),
                'not_found'                 => sprintf(esc_html__('No %s found.', 'qibla-directory'), $plural),
            ),
            'public'  => true,
