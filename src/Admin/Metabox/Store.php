...
use QiblaDirectory\Listings\ListingLocationStore;
use QiblaDirectory\Admin\AbstractMetaboxStore;
use QiblaDirectory\User\User;
...
class Store extends AbstractMetaboxStore
{
    ...
    public function meta($postID, $post, $update)
    {
        ...
        if (! current_user_can('edit_posts') && ! User::isListingsManager()) {
            wp_die(esc_html__('You are not allowed to store post meta.', 'qibla-directory'));
        }
        ...
        foreach ($this->metaBoxes as $metabox) :
            ...
            if (! empty($fields)) :
                ...
                foreach ($validate['valid'] as $name => $value) :
                     ...
                     */
                    do_action('qibla_directory_metabox_after_store_meta', $name, $value, $post, $update, $oldValue, $this);
                endforeach;
